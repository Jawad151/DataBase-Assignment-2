...
-----------------------------------------------------------------------------------------------------
--QUEURIES STARTS FROM HERE


--3
--Write a SQL query to find the registration numbers of airplanes that have never undergone maintenance.

select SERVICE_.reg_id as PlaneReg
from SERVICE_
inner join airplane_ on SERVICE_.reg_id = airplane_.reg_no AND SERVICE_.hours_spent = 0
...
--4
--Write a SQL query to find the names and addresses of corporations that own airplanes with a capacity greater than 200.
select Corporation.Name,Corporation.Address2,plane_type.capacity
FROM Corporation
INNER JOIN plane_type ON Corporation.Owned = plane_type.model_no  and plane_type.capacity > 200  
...
--6
--Write a SQL query to find the top 5 employees with the highest total number of maintenance hours worked.

select distinct top 5 EMPLOYEE_.name, sum(SERVICE_.hours_spent) as Total_Working_Hours
from EMPLOYEE_
Join SERVICE_ ON EMPLOYEE_.employee_id = SERVICE_.emp_id
group by EMPLOYEE_.name
...
select airplane_.reg_no as PlaneReg,airplane_.plane_t as PlaneType
from  airplane_
inner join SERVICE_  on  airplane_.reg_no =  SERVICE_.reg_id and SERVICE_.date_of_maintenance >=DATEADD(DAY, -7, GETDATE());

------------------------------------------------------------------------------------------------------------

--8
--Write a SQL query to find the names and phone numbers of all owners who have purchased a plane in the past month.
 
select owner.name ,owner.contact_info
from OWNER
INNER JOIN OWNS ON OWNER.owner_id = OWNS.owner_id
WHERE OWNS.purchase_date >= DATEADD(month, -1, GETDATE());
...
select top 5 Pilot_.Lic_num , count(*) as PlanesAuthorized
from Pilot_
Join plane_type on Pilot_.plane = plane_type.model_no
group by Pilot_.Lic_num

------------------------------------------------------------------------------------------------------------

--10
--Write a SQL query to find the location and capacity of the hangar with the most available space.
SELECT top 5 Hanger.location , Hanger.capacity
FROM Hanger 
...
select Corporation.Owned ,count(*) as PlaneOwned_Corp
from Corporation
inner join plane_type on Corporation.Owned = plane_type.model_no
group by Corporation.Owned
order by count(*) desc;
...
select airplane_.plane_t  , AVG(SERVICE_.hours_spent) as Avg
from airplane_
inner join SERVICE_ on airplane_.reg_no = SERVICE_.reg_id
group by airplane_.plane_t
...
select DISTINCT OWNER.name,airplane_.plane_t,SERVICE_.hours_spent
from owner
inner join owns on OWNER.owner_id = OWNS.owner_id
inner join EMPLOYEE_ on EMPLOYEE_.employee_id = OWNER.owner_id
inner join SERVICE_ on SERVICE_.emp_id = EMPLOYEE_.employee_id
inner join airplane_ on airplane_.reg_no = SERVICE_.reg_id
where
SERVICE_.hours_spent = 0 AND EMPLOYEE_.position  != 'Director of Maintenance'
...
SELECT OWNER.name ,OWNER.contact_info
FROM OWNER 
INNER JOIN Corporation ON OWNER.address = Corporation.Address2
...
select Pilot_.SSN_P AS Pilot_Name
from pilot_
join plane_type on Pilot_.plane = plane_type.model_no
join airplane_ on airplane_.plane_t = plane_type.model_no 
join SERVICE_ on airplane_.reg_no = SERVICE_.reg_id
  
WHERE SERVICE_.date_of_maintenance  = GETDATE();
...
SELECT distinct airplane_.reg_no ,airplane_.plane_t,SERVICE_.hours_spent
FROM airplane_
INNER JOIN Corporation ON airplane_.plane_t = Corporation.Owned
inner join SERVICE_ on airplane_.reg_no = SERVICE_.reg_id and SERVICE_.hours_spent = 0
inner join EMPLOYEE_ on EMPLOYEE_.Shift__ = 'Day'
...
from owner
join OWNS on  OWNER.owner_id = OWNS.owner_id 
join Corporation on OWNER.owner_id = Corporation.EMP_id 
join plane_type on Corporation.Owned = plane_type.model_no
WHERE OWNS.purchase_date >= DATEADD(month, -1, GETDATE());
...
--19
--a Query to find the total number of planes stored in each hangar.
select plane_type.model_no,airplane_.hanger
from plane_type
inner join airplane_ on plane_type.model_no = airplane_.plane_t
...
select plane_type.model_no ,count(*) as NumberofPlanes
from plane_type
inner join airplane_ on plane_type.model_no = airplane_.plane_t
...
select SERVICE_.reg_id as PlaneID,SERVICE_.hours_spent as Service
from  SERVICE_
inner join airplane_ on SERVICE_.reg_id  = airplane_.reg_no
...
select Shift__ ,AVG(Salary) as Avg_Salary
from Employee_
group by Shift__
...
Select owner.name  ,count(*) as Own_Planes
from owner
inner join owns on OWNER.owner_id = OWNS.owner_id
GROUP BY OWNER.Name;
...
--25
--
--THIS QUERY IS NECESSARY FOR MANIPULATING THE QUERIRES FOR CALCULATING SALARY OF AVERAGE EMPLOYEE

select  AVG(Salary) as Avg_Sal 
from EMPLOYEE_


--THIS QUERY IS FOR FINDING THE TOP 5 AIRPLANE HAVING MOST NUMBERS OF HOURS SPENT ON MAINTANCE
select top 5 SERVICE_.reg_id as PlaneID ,airplane_.plane_t as PlaneType,SERVICE_.hours_spent,EMPLOYEE_.employee_id
from SERVICE_
Inner join airplane_ on airplane_.reg_no = SERVICE_.reg_id
Inner join EMPLOYEE_ on EMPLOYEE_.employee_id = SERVICE_.emp_id
order by SERVICE_.hours_spent desc



--this query is necessary for selecting the top 5 corporations
select distinct TOP 5 EMPLOYEE_.employee_id , Corporation.Name,  Corporation.Owned as PlaneType,SERVICE_.hours_spent
from Corporation
inner join EMPLOYEE_ on EMPLOYEE_.employee_id = Corporation.EMP_id
inner join SERVICE_ on EMPLOYEE_.employee_id = SERVICE_.emp_id
ORDER BY SERVICE_.hours_spent desc




--this query will bring the airplanes having date of maintainance of planes in last 1 year

select  SERVICE_.reg_id, airplane_.plane_t,SERVICE_.date_of_maintenance
from SERVICE_
inner join airplane_ on SERVICE_.reg_id = airplane_.reg_no
WHERE SERVICE_.date_of_maintenance >= DATEADD(YEAR, -1, GETDATE())
order by SERVICE_.date_of_maintenance desc
